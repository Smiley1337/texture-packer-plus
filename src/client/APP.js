...
class APP {
    ...
    async doExport() {
        ...
        let finalObject = {
            "textures": [],
            "meta": {
                "app": "darkkajontop", // i dont remember the app name from yesterday spritesheet parser good
		        "version": "6.6.6"
            }
        };
        ...
        for (let item of this.packResult) {
            ...
            var packJson = await startExporter(exporter, item.data, options);
            var parsed = JSON.parse(packJson);
            parsed.textures[0].image = `${parsed.textures[0].image}.${this.packOptions.textureFormat}`
            finalObject.textures.push(parsed.textures[0]);

            ix++;
        }

        files.push({
            name: this.packOptions.textureName + "." + this.packOptions.exporter.fileExt,
            content: JSON.stringify(finalObject, null, 4)
        });
